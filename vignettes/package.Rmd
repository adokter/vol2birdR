...
install_mistnet()
```
This will download libtorch from https://download.pytorch.org as well as a wrapper library from our AWS that enables the mistnet functionality. The mistnet model has to be manually downloaded and
...
```R
library(vol2birdR)
config<-new(Vol2BirdConfig)
config$verbose<-TRUE
config$useMistNet<-TRUE
processor<-new(Vol2Bird)
